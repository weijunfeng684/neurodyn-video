"""
Same evaluation flow as evaluate.py for the ResNet-feature checkpoint path.

Uses train_gru_resnet constants so label order matches that training run.
"""
...
def plot_confusion_matrix(cm: np.ndarray, class_names: list[str], output_path: Path) -> None:
    plot_confusion_matrix_fancy(cm, class_names, output_path, title="Confusion matrix (ResNet + GRU)")
...
def main() -> None:
    ...
    np.savez_compressed(cache_dir / "confusion_resnet.npz", cm=cm.astype(np.float64))
    plot_confusion_matrix(cm, CLASS_NAMES, OUTPUT_DIR / "confusion_matrix_resnet.png")
